...
NSString *const SELECTABLE_CUSTOM_SELECTOR = @"SELECTABLE_CUSTOM_SELECTOR_";
...
-(void) _handleGesture
{
    ...
    for(NSString *menuItemName in self.menuItems) {
        NSString *sel = [NSString stringWithFormat:@"%@%@", SELECTABLE_CUSTOM_SELECTOR, menuItemName];
...
- (void)forwardInvocation:(NSInvocation *)invocation
{
    ...
    NSRange match = [sel rangeOfString:SELECTABLE_CUSTOM_SELECTOR];
    if (match.location == 0) {
